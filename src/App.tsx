...
import { categories, mockFlyers, mockUsers, businessRegistry } from './data/mockData';
import { Flyer, Category, User, BusinessRegistry, GeolocationState, Review } from './types';
import { getCurrentLocation, sortFlyersByDistance } from './utils/geolocation';
import { checkPremiumRequired, checkBusinessNameAvailable } from './utils/subscription';
...
function App() {
  ...
  const [businessNames, setBusinessNames] = useState<BusinessRegistry[]>(businessRegistry);
  const [currentUser, setCurrentUser] = useState<User>(mockUsers[0]);
  const [isPostModalOpen, setIsPostModalOpen] = useState(false);
  const [isPremiumModalOpen, setIsPremiumModalOpen] = useState(false);
  const [isSportsModalOpen, setIsSportsModalOpen] = useState(false);
  const [isMapViewOpen, setIsMapViewOpen] = useState(false);
  const [premiumModalReason, setPremiumModalReason] = useState<'expired' | 'required' | 'upgrade'>('required');
  ...
  useEffect(() => {
    const initializeLocation = async () => {
      try {
        ...
        setCurrentLocation(`${location.lat.toFixed(2)}, ${location.lng.toFixed(2)}`);
      } catch (error) {
        setGeolocation({
          userLocation: null,
          isLoading: false,
  ...
  const sortedFlyers = geolocation.userLocation
    ? sortFlyersByDistance(flyers, geolocation.userLocation.lat, geolocation.userLocation.lng)
    : flyers;

  const nearbyFlyers = geolocation.userLocation
  ...
  const handlePostFlyer = (flyerData: any) => {
    ...
    const monthlyCount = currentUser.lastPostMonth === currentMonth ? currentUser.monthlyPostsCount : 0;
    ...
    const newFlyer: Flyer = {
      ...
      imageUrl: flyerData.imageUrl || 'https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg?auto=compress&cs=tinysrgb&w=500',
  ...
  const currentMonth = new Date().toISOString().slice(0, 7);
  const monthlyPostsCount = currentUser.lastPostMonth === currentMonth ? currentUser.monthlyPostsCount : 0;
